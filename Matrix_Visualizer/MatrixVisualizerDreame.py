...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        np.set_printoptions(threshold=np.inf)  # 数据全部打印
        ...
        self.setWindowTitle('贝氪测试治具——墨现科技')
        self.setWindowIcon(QIcon('matrix.png'))
        pg.setConfigOptions(antialias=True)

        # 设置Dock位置
        # give this dock the minimum possible size
        d1 = Dock("压力分布图", size=(700, 450), closable=True, fontSize=20)
        ...
        self.area.addDock(d8, 'right', d1)
        self.area.addDock(d2, 'above', d8)
        self.area.addDock(d3, 'bottom', d1)
        self.area.addDock(d4, 'bottom', d8)
        ...
        self.min_map.setValue(0.02)
        self.max_map.setValue(2.60)
        self.min_map.setRange(0, 100000)
        self.max_map.setRange(0, 100000)
        self.min_map.valueChanged.connect(self.update_map_level)
        self.max_map.valueChanged.connect(self.update_map_level)
        ...
        self.d1_group.buttonClicked[int].connect(self.map_color_check)
        ...
        self.trans_data.connect(self.update_whole_time)
        ...
        self.matrix_table.setColumnCount(12)  # 设置表格一共有10列
        self.matrix_table.setRowCount(12)  # 设置表格一共有10列
        ...
        # 倒计时记录
        excel_row_label = QLabel('起始行')
        excel_col_label = QLabel('起始列')
        countdown_label = QLabel('倒计时')
        percent_label = QLabel('变化量')
        count_id_label = QLabel('编号')
        self.excel_row_data = QLineEdit()
        self.excel_col_data = QLineEdit()
        exl_reg = QRegExp('[A-z]+$')
        exl_validator = QRegExpValidator(self)
        exl_validator.setRegExp(exl_reg)
        self.excel_col_data.setValidator(exl_validator)
        self.countdown_data = QDoubleSpinBox()
        self.percent_data = QDoubleSpinBox()
        self.count_id = QLineEdit()
        self.re_position_btn = QPushButton('重定位')
        self.re_position_btn.clicked.connect(self.re_position)
        self.countdown_btn = QPushButton('开始倒计时')
        self.countdown_btn.setShortcut('shift+x')
        ...
        w13.addWidget(excel_row_label, row=0, col=0)
        w13.addWidget(self.excel_row_data, row=0, col=1)
        w13.addWidget(excel_col_label, row=0, col=2)
        w13.addWidget(self.excel_col_data, row=0, col=3)
        w13.addWidget(countdown_label, row=1, col=0)
        w13.addWidget(self.countdown_data, row=1, col=1)
        w13.addWidget(count_id_label, row=1, col=2)
        w13.addWidget(self.count_id, row=1, col=3)
        w13.addWidget(percent_label, row=2, col=0)
        w13.addWidget(self.percent_data, row=2, col=1)
        w13.addWidget(self.re_position_btn, row=2, col=2)
        w13.addWidget(self.countdown_btn, row=2, col=3)
        ...
        self.bps_combobox.addItems(['460800', '115200', '230400'])
    ...
    def check_sum(self):
        while self.loop:
            try:
                ...
                if sum(decimal_list) == check_data:  # 除开校验位，自身加和
                    # matrix_data = double_list[self.head_len:-2]
                    reform_data = double_list[self.head_len:-2]
        ...
        while self.loop:
            try:
                ...
                # 显示电阻值
                    ...
                    save_list = np.maximum(save_list, 0.001)
                    save_list = np.divide(np.multiply(np.subtract(self.voltage_num, save_list), self.resistance_num),
                                          save_list)
                    save_list = np.around(save_list, 1)
                    # save_list = save_list - self.internal_resistance

                list_len = len(save_list)
                matrix_len = self.row_num * self.col_num  # 矩阵行列
                ...
                self.test_tool_list = self.matrix_list
    ...
    def update_map(self, signal):
        ...
        area1 = signal['matrix_list'][0][0]
        area2 = signal['matrix_list'][0][1]
        area3 = signal['matrix_list'][0][2]
        area4 = signal['matrix_list'][0][3]
        area5 = signal['matrix_list'][0][4]
        area6 = signal['matrix_list'][0][5]
        area7 = signal['matrix_list'][0][6]
        area8 = signal['matrix_list'][0][7]
        area9 = signal['matrix_list'][0][8]

        tools = np.array([[area3, area3, area4, area2, area4, area1, area1],
                          [area3, area3, area4, area2, area4, area1, area1],
                          [area3, area3, area4, area4, area4, area1, area1],
                          [area6, area6, area4, area4, area4, area5, area5],
                          [area6, area6, area9, area8, area7, area5, area5],
                          [area9, area9, area9, area8, area7, area7, area7],
                          [area9, area9, area9, area8, area7, area7, area7]])

        self.img.setImage(tools, autoLevels=self.auto_map, levels=self.map_level)
    ...
    def update_whole_time(self, signal):
    ...
    def update_histogram(self, signal):
        ...
        y, x = np.histogram(signal['save_list'], bins=num_interval, range=(range_min, range_max),
                            density=self.histogram_density)
    ...
    def update_matrix(self, signal):
        ...
        for r in range(0, row):
            for c in range(0, col):
                cur_x = signal['matrix_list'][-r-1][c]
                # poly
                self.matrix_table.setItem(r, c, QTableWidgetItem(
                    str(round(poly[c][0]*(cur_x**2)+poly[c][1]*cur_x))))
                # custom
                # self.matrix_table.setItem(r, c, QTableWidgetItem(
                #     str(round(custom[0][0]*math.exp(cur_x*custom[0][1])+custom[0][2]))))
    ...
    def timer_insert(self):
        try:
            ...
            datas = (time_database, name_database, weight_database, self.copy_data["num_data"],
                     self.copy_data['avg_data'], self.copy_data['mid_data'], self.copy_data['var_data'],
                     self.copy_data['std_data'],
                     self.copy_data['range_data'], self.copy_data['max_data'], self.copy_data['min_data'],
                     str(self.copy_data['matrix_list']))
            self.c.execute(sql, datas)
            self.conn.commit()
        except Exception as e:
            self.printf("上传到数据库失败：" + str(e))
    ...
    #         self.percent_num = (100 - self.percent_data.value()) * 0.01 * np.array(self.copy_data['save_list'])
    #         print(self.percent_num)
    #         # 数据库
    ...
    def re_position(self):
        try:
            self.wb_row = int(self.excel_row_data.text())
            self.wb_col = list(self.excel_list.values()).index(self.excel_list[self.excel_col_data.text().upper()])
        except:
            self.printf("请输入正确的EXCEL坐标")
        else:
            self.printf('已重定起始写入位')
    ...
    def update_countdown(self):
        if self.countdown_signal:
            ...
            # 打开Excel
            try:
                self.wb = load_workbook('初始值测试.xlsx')
    ...
    def count_compare(self, signal):
        for i in range(0, len(signal['save_list'])):
            if signal['save_list'][i] < self.percent_num[i]:
    ...
    #         datas = (self.count_start_time, self.count_id.text(), self.weight_data.text(),
    #         self.countdown_data.value(),self.percent_data.value(), self.copy_data['min_data'])
    ...
    def count_insert(self):  # excel
        try:
            col_list = list(self.excel_list.values())
            id_str = str(self.ws.cell(self.wb_row, 1))
            id_name = id_str[id_str.find('.') + 1: -1]
            weight = '错误！'
            region = 0
            if 3 >= self.wb_col >= 0:
                weight = '50g'
                region = col_list[self.wb_col] - 1
            elif 7 >= self.wb_col >= 4:
                weight = '100g'
                region = col_list[self.wb_col] - 7
            elif 11 >= self.wb_col >= 8:
                weight = '200g'
                region = col_list[self.wb_col] - 13
            elif 15 >= self.wb_col >= 12:
                weight = '500g'
                region = col_list[self.wb_col] - 19
            self.printf(id_name + '-' + weight + '-' + str(region))
            self.ws.cell(self.wb_row, col_list[self.wb_col]).value = str(self.copy_data['min_data'])
            self.wb.save('初始值测试.xlsx')
            self.printf("倒计时已完成")
        except Exception as e:
            print(e)
            self.printf("倒计时写入Excel失败：" + str(e))
        else:
            if self.wb_col == len(col_list) - 1:
                self.wb_col = 0
                self.wb_row += 1
            else:
                self.wb_col += 1
    ...
    def screen_shot(self):
        try:
            ...
            hwnd = FindWindow(None, "贝氪测试治具——墨现科技")
